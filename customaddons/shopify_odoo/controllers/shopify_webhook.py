...
class ShopifyWebhook(Controller):
    ...
    @route('/shopify/<int:shopify_shop_id>/products/create', type='json', auth='public', csrf=False, website=False, methods=['POST'])
    def create_product_callback(self, shopify_shop_id=None, **post):
        try:
            # current_id = request.env.uid
            search_user = request.env['res.users'].sudo().search([('id', '=', shopify_shop_id)], limit=1)
            search_shop = request.env['s.shop'].sudo().search([('shop_base_url', '=', search_user.login)])
            ...
            if product.get('id'):
                for pro_val in product['variants']:
                    product_vals = {
                        ...
                        'is_published': True,
                        'categ_id': 1,
                        'shopify_user_id': shopify_shop_id,
    ...
    @route('/shopify/<int:shopify_shop_id>/products/update', type='json', auth='public', csrf=False, website=False, methods=['POST'])
    def update_product_callback(self, shopify_shop_id=None, **post):
        try:
            # current_id = request.env.uid
            search_user = request.env['res.users'].sudo().search([('id', '=', shopify_shop_id)], limit=1)
            search_shop = request.env['s.shop'].sudo().search([('shop_base_url', '=', search_user.login)])
    ...
    @route('/shopify/<int:shopify_shop_id>/orders/create', type='json', auth='public', csrf=False, website=False, methods=['POST'])
    def create_order_callback(self, shopify_shop_id=None, **post):
        try:
            order = request.jsonrequest
            if order['id']:
                # todo: Search and Link or Create Customer in Odoo
                if 'customer' not in order:
                    search_customer = request.env['res.partner'].sudo().search([('phone', '=', order['shipping_address']['phone'])])
                ...
                link_partner = request.env['res.partner'].sudo().search([('phone', '=', order['shipping_address']['phone'])], limit=1)
    ...
    @route('/shopify/<int:shopify_shop_id>/orders/updated', type='json', auth='public', csrf=False, website=False, methods=['POST'])
    def update_order_callback(self, shopify_shop_id=None, **post):
        try:
            order = request.jsonrequest
            if order.get('id'):
                # current_id = request.env.uid
                ...
                search_order = request.env['sale.order'].sudo().search([('shopify_order_id', '=', order.id)], limit=1)
                update_record = search_order.sudo().create(order_vals)
                if update_record:
                    return Response("OK", status=200)
                else:
                    return Response("Error", status=401)
